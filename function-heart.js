...
while (i < v ) {
	...
	while ( k < v ) { 
		f[k++] = { // create new particle
			...
			R : (1 - k/v)  + 1, // radius
			S : R() + 1, // acceleration 
			q : ~~(R() * v), // target node on heart
			//D : R()>.5?1:-1,
			D : i%2*2-1, // direction around heart 
			F : R() * .2 + .7, // friction
...
window.addEventListener('wheel', function(e) {
    if (e.ctrlKey) {
        e.preventDefault(); // Ctrl + Scroll ile zoom'u engeller
    }
}, { passive: false });

document.addEventListener('gesturestart', function(e) {
    e.preventDefault(); // Mobil pinch-to-zoom'u engeller
});